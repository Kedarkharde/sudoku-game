...
def p1():
    ...
    class Example(Frame):
        def __init__(self, master, *pargs):
            ...
            self.image = Image.open("F:/travelo/IMAGES/sudoku.jpg")
    ...
    # lbl=Label(frame1,image=image,height=500,width=500).grid()
...
def p2(previousf):
    ...
    b23 = Button(frame2, text="MENU", command=lambda: menu(frame2), bg="alice blue", fg="blue2")
    b23.grid(padx=10, pady=20, row=26, column=3, columnspan=4)
    b23['font'] = myFont

    b24 = Button(frame2, text="EXIT", command=lambda: go(frame2), bg="hot pink", fg="red")
...
def p4(previousf):
    ...
    b44 = Button(frame4, text="BACK", command=lambda: menu(frame4), bg="DarkOrchid1", fg="cyan", width=12)
    b44.grid(padx=10, pady=20, row=28, column=8, columnspan=10)
    b44['font'] = myFont

    b45 = Button(frame4, text="EXIT", command=lambda: go(frame4), bg="DarkOrchid1", fg="red2", width=12)
    b45.grid(padx=10, pady=20, row=28, column=22, columnspan=10)
    b45['font'] = myFont


def difficulty(frame,v):
    dest(frame)
    ...
    be1 = Button(frame5, text="QUESTION 1", command=lambda: calling(window, frame5, v, 1, difficulty), bg="cyan", fg="deep pink", width=25)
    be1.grid(padx=10, pady=20, row=25, column=11, columnspan=10)
    be1['font'] = myFont

    be2 = Button(frame5, text="QUESTION 2", command=lambda: calling(window, frame5, v, 2, difficulty), bg="deep pink", fg="yellow", width=25)
    be2.grid(padx=10, pady=20, row=26, column=11, columnspan=10)
    be2['font'] = myFont

    be3 = Button(frame5, text="QUESTION 3", command=lambda: calling(window, frame5, v, 3, difficulty), bg="OliveDrab1", fg="blue2", width=25)
    be3.grid(padx=10, pady=20, row=27, column=11, columnspan=10)
    be3['font'] = myFont

    be4 = Button(frame5, text="BACK", command=lambda: back2(frame5), bg="orchid1", fg="black", width=10)
    be4.grid(padx=10, pady=20, row=28, column=5, columnspan=10)
    be4['font'] = myFont

    be5 = Button(frame5, text="EXIT", command=lambda: go(frame5), bg="orchid1", fg="red", width=10)
...
window.geometry('770x570')
window.title("Sudoku")
#image= ImageTk.PhotoImage(Image.open("F:/travelo/IMAGES/sudoku.jpg"))
